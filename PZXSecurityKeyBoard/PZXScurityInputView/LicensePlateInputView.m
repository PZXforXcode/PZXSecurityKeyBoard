...
#define License_WIDTH [UIScreen mainScreen].bounds.size.width
#define License_HEIGHT [UIScreen mainScreen].bounds.size.height
#define INPUTHEIGHT 180 * (License_HEIGHT/568)
...
-(void)createInputV{
    ...
    self.frame = CGRectMake(0, License_HEIGHT-INPUTHEIGHT, License_WIDTH, INPUTHEIGHT);
    ...
    _provinceView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, License_WIDTH, INPUTHEIGHT)];
    [self addSubview:_provinceView];
    
    
    CGFloat provinceWidth = (License_WIDTH) / 9;//单行数量为9时所占宽度
    ...
    for (int i = 0; i < _provinceArr.count+2; i++) {//数量+2为后面添加删除确定键预留
        ...
        if (i < _provinceArr.count) {
            ...
            provinceBtn.frame = CGRectMake(provinceFrameX, 1.5 + provinceHeight * 3, License_WIDTH-provinceFrameX- 1.5, provinceHeight - 3);
    ...
    UIView *letterView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, License_WIDTH, INPUTHEIGHT)];
    letterView.hidden = YES;
    self.letterView = letterView;
    [self addSubview:letterView];
    
    CGFloat letterWidth = (License_WIDTH) / 10;
    ...
    for (int i = 0; i < _letterArr.count+2; i++) {
        ...
        if (i < _letterArr.count) {
            ...
            letterBtn.frame = CGRectMake(letterFrameX, 1.5 + letterHeight * 3, License_WIDTH-letterFrameX- 1.5, letterHeight - 3);
    ...
    if (index == 33){//确认
        ...
        if (_plateStr.length<8) {
            ...
            if (_plateStr.length>0) {
                NSArray * provinceArr = _provinceArr;
...
#pragma mark 隐藏键盘
- (void)putAway{
    UIResponder *firstResponder =  (UIResponder *)_keyInput;
    if (firstResponder) {
